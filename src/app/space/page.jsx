...
export default function HomePage() {
  const sections = [
    ...
    { title: "אפליקציות לניהול וייעול העסק", text: "אפליקציות לייעול, שמירה וארגון מידע בסביבה מקומית או בענן. אפליקציות לניהול מלאי, משימות, לוחות זמנים, מעקב ביצוע ושליטה תפורים לצרכיך לפי מידה" },
  ...
  const buttonStyles = [
    { from: "#001060", to: "#001f9a", innerFrom: "#302fff", innerTo: "#101043" },
    { from: "#86198f", to: "#4a044e", innerFrom: "#f472b6", innerTo: "#86198f" },
    { from: "#155e75", to: "#164e63", innerFrom: "#67e8f9", innerTo: "#155e75" },
    { from: "#78350f", to: "#451a03", innerFrom: "#fde68a", innerTo: "#78350f" },
    { from: "#be123c", to: "#881337", innerFrom: "#fda4af", innerTo: "#be123c" },
    { from: "#4d7c0f", to: "#365314", innerFrom: "#d9f99d", innerTo: "#4d7c0f" },
  ];
...
const [visibleButtons, setVisibleButtons] = useState([]);
  ...
  return (
    <main className="bg-black text-white overflow-x-hidden relative" style={{ height: `${fullHeight}px` }}>
      ...
      {/* כפתורים דינאמיים לפי הופעת הסקשן */}
      <div className="fixed right-20 z-[1001] flex flex-col items-center gap-[1.75rem] mt-40">
        {visibleButtons.map((idx) => {
          ...
          return (
            <div
              key={idx}
              className={`w-24 h-24 rounded-[20%] bg-gradient-to-br from-[${style.from}] to-[${style.to}] shadow-[0_0_30px_rgba(255,180,100,0.35)] hover:shadow-[0_0_45px_rgba(255,180,100,0.6)] hover:scale-105 transition-all duration-300 cursor-pointer flex items-center justify-center`}
            >
              <div
                className={`w-[80%] h-[80%] rounded-[20%] bg-gradient-to-br from-[${style.innerFrom}] to-[${style.innerTo}] flex items-center justify-center text-center p-2`}
                style={{
                  boxShadow: "inset 0 4px 6px rgba(0,0,0,0.6), inset 0 -3px 5px rgba(255,255,255,0.08)",
                }}
              >
